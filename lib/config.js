...
exports.API_BASE_URL = exports.API_URL = exports.appConfig = exports.API_VERSION_PREFIX = exports.APP_ENVIRONMENTS = void 0;
exports.buildApiUrl = buildApiUrl;
exports.APP_ENVIRONMENTS = ["local", "production"];
exports.API_VERSION_PREFIX = "/api/v1";
function readAppEnv() {
    const raw = (process.env.NEXT_PUBLIC_APP_ENV ?? "").trim().toLowerCase();
    if (!raw) {
        throw new Error("[api-config] NEXT_PUBLIC_APP_ENV is missing. " +
            `Set it to one of: ${exports.APP_ENVIRONMENTS.join(" | ")}. ` +
            "Then restart `npm run dev` (or rebuild for production).");
    }
    if (raw !== "local" && raw !== "production") {
        throw new Error(`[api-config] Invalid NEXT_PUBLIC_APP_ENV=${JSON.stringify(raw)}. ` +
            `Supported values: ${exports.APP_ENVIRONMENTS.join(" | ")}.`);
    }
    return raw;
}
function readApiUrl() {
    ...
    if (!raw) {
        throw new Error("[api-config] NEXT_PUBLIC_API_URL is missing. " +
            "Set the backend base URL (e.g. http://localhost:4000 for local, " +
            "https://server.shohojhisab.com for production). " +
            "Then restart `npm run dev` (or rebuild for production).");
    }
    if (!/^https?:\/\//i.test(raw)) {
        throw new Error(`[api-config] Invalid NEXT_PUBLIC_API_URL=${JSON.stringify(raw)}. ` +
...
const API_BASE_URL = `${API_URL}${exports.API_VERSION_PREFIX}`;
...
if (APP_ENV === "local" && !isLocalhostLike(API_URL)) {
    console.warn(`[api-config] WARNING: NEXT_PUBLIC_APP_ENV=local but NEXT_PUBLIC_API_URL=${API_URL} ` +
        "points at a remote backend. Local work may hit production data.");
}
if (APP_ENV === "production" && isLocalhostLike(API_URL)) {
    console.warn(`[api-config] WARNING: NEXT_PUBLIC_APP_ENV=production but NEXT_PUBLIC_API_URL=${API_URL} ` +
...
function buildApiUrl(path) {
    ...
    const baseWithoutApi = base.endsWith("/api") && withSlash.startsWith("/api/") ? base.slice(0, -4) : base;
    if (withSlash.startsWith(`${exports.API_VERSION_PREFIX}/`) || withSlash === exports.API_VERSION_PREFIX) {
