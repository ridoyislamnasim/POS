...
export function OrderCustomerPicker({
  ...
  onChange: (c: OrderCustomer | null) => void;
}) {
  const listId = useId();
  ...
  const search = useQuery({
    queryKey: ["so-customer-search", debounced],
    queryFn: () => api<CustomerHit[]>(`/api/v1/customers/search?q=${encodeURIComponent(debounced.trim())}&limit=20`),
    enabled: open,
    staleTime: 15_000,
  ...
  function selectHit(row: CustomerHit) {
    onChange({ id: row.id, name: row.name, phone: row.phone, email: row.email ?? null, creditDue: row.creditDue ?? 0 });
    setOpen(false);
    setQ("");
  ...
  const due = dueAmount(value?.creditDue);
  const openPicker = () => {
    setOpen(true);
  ...
  return (
    <div ref={rootRef} className="relative z-10">
      {value ? (
        ...
        >
          <span className="flex min-w-0 items-center gap-1.5">
            ...
            <span className="truncate text-muted-foreground">{value.phone}</span>
            {due > 0 ? <span className="shrink-0 font-medium tabular-nums text-amber-700 dark:text-amber-400">Due {moneyLabel(due)}</span> : null}
          </span>
        ...
        >
          <span>Walk-in</span>
          <span className="tabular-nums">+ customer</span>
        </button>
      )}
      ...
      {open ? (
        <div className="mt-1">
          <input
            ...
            onKeyDown={onKeyDown}
            placeholder="Name, phone, email, or ID — walk-in allowed"
      ...
      {open ? (
        <div id={listId} role="listbox" className="absolute z-50 mt-1 max-h-[min(18rem,60vh)] w-full overflow-auto rounded-md border bg-popover text-popover-foreground shadow-lg">
          {search.isFetching ? <div className="px-3 py-2 text-xs text-muted-foreground">Searching…</div> : null}
          {!search.isFetching && rows.length === 0 ? <div className="px-3 py-3 text-sm text-muted-foreground">No customer matches — walk-in or create new</div> : null}
          {rows.length > 0
            ? rows.map((row, i) => {
                ...
                return (
                  <button
                    ...
                    className={`flex min-h-12 w-full flex-col items-start gap-0.5 px-3 py-2 text-left text-sm ${i === active ? "bg-muted" : "hover:bg-muted/70"}`}
                  ...
                  >
                    ...
                    <span className="text-xs text-muted-foreground">
                      {row.phone} <span className="ml-1 font-mono">· {row.id.slice(-8)}</span>
                      {row.email ? ` · ${row.email}` : ""}
      ...
      {detailsOpen && value ? (
        <Modal title="Customer details" size="sm" onClose={() => setDetailsOpen(false)}>
          <div className="rounded-md border bg-card px-3 py-2">
            ...
            {value.email ? <div className="text-sm text-muted-foreground">{value.email}</div> : null}
          </div>
          <dl className="mt-2 space-y-1 text-xs">
            <div className="flex items-center justify-between gap-2">
              <dt className="text-muted-foreground">Customer ID</dt>
              <dd className="font-mono">{value.id}</dd>
            </div>
            <div className="flex items-center justify-between gap-2">
              <dt className="text-muted-foreground">Credit due</dt>
              <dd className={`tabular-nums ${due > 0 ? "font-semibold text-amber-700 dark:text-amber-400" : ""}`}>{moneyLabel(due)}</dd>
            </div>
          </dl>
          <div className="mt-3 flex gap-2">
            <button type="button" className={btnGhost + " flex-1"} onClick={() => { onChange(null); setDetailsOpen(false); }}>
              Walk-in
            </button>
            <button type="button" className={btnGhost + " flex-1"} onClick={() => { setDetailsOpen(false); openPicker(); }}>
              Change
            </button>
...
function CreateCustomerModal({
  ...
  const create = useMutation({
    mutationFn: async () => {
      ...
      return api<CustomerHit>("/api/v1/customers", {
        method: "POST",
        body: JSON.stringify({ name: name.trim(), phone: phone.trim(), email: email.trim() || undefined, createOnly: true }),
      });
    },
    onSuccess: (row) => {
  ...
  return (
    <Modal title="Add customer" size="sm" onClose={onClose}>
      <p className="mb-2 text-xs text-muted-foreground">Creates once and attaches to this order. Existing phone numbers are selected, not duplicated. Walk-in allowed — leave empty if needed.</p>
